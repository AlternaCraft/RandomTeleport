 ...
 * @author Julián
 */
public class HandleChat implements Listener, EventExecutor {
    ...
    public HandleChat(RandomTeleport plugin) {
        this.plugin = plugin;
    }
    ...
    public void onPlayerChat(AsyncPlayerChatEvent e) {
        ...
        for (int i = 0; i < lista.size(); i++) {
            ...
            if (adm.getJugador().getName().equals(e.getPlayer().getName())) {            
    ...
    public void marcas(Configuracion adm, AsyncPlayerChatEvent e) {        
        ...
        if (!adm.isParte1()) {                        
            ...
            s1 = new Punto((int) l.getX(), HandleTeleport.Y, (int) l.getZ());
            adm.setParte6(true);
            ...
            s2 = new Punto((int) l.getX(), HandleTeleport.Y, (int) l.getZ());
            adm.getL().getSzonas().add(new Zona(s1,s2));
            adm.setParte6(false);
        ...
        if (!adm.isParte5()) {
            ...
            if (!adm.getL().getSzonas().isEmpty()) {
                for (int j = 0; j < adm.getL().getSzonas().size(); j++) {
                    Zona sz = adm.getL().getSzonas().get(j);
